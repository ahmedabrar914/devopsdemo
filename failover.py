...
def is_re_establish_already_done(checks: dict) -> bool:
    ...
    if p.get("mode") != "secondary" or s.get("mode") != "primary":
        return False

    p_conn = (p.get("connection_state") or "").lower()
    s_known_secondaries = s.get("known_secondaries_count", 0)

    if p_conn in {"connected", "ready", "stream-wals"}:
        return True
    if s_known_secondaries >= 1:
        return True

    return False


def run_controlled_failover(ssl_context, primary_token: str, batch_token: str):
    checks = validate_current_state(ssl_context, primary_token)

    if is_controlled_failover_already_done(checks):
        return {
            "skipped": True,
            "message": "Controlled failover already completed. No action required.",
            "checks": checks
        }

    steps = {}

    print("Controlled failover: Step 1 demote cluster A")
    st1, d1 = vault_post(
        ssl_context,
        PRIMARY_ADDR,
        primary_token,
    ...
    print(f"Controlled failover step1 HTTP: {st1}")
    require_ok("controlled_failover_step1_demote_cluster_a", st1, d1)
    steps["step1_demote_cluster_a"] = {"http": st1, "response": d1}

    print("Controlled failover: Step 2 promote cluster B")
    st2, d2 = vault_post(
        ssl_context,
        SECONDARY_ADDR,
        primary_token,
    ...
    print(f"Controlled failover step2 HTTP: {st2}")
    require_ok("controlled_failover_step2_promote_cluster_b", st2, d2)
    steps["step2_promote_cluster_b"] = {"http": st2, "response": d2}

    print(f"Sleeping {POST_PROMOTE_SLEEP_SECONDS}s after controlled failover promote")
...
def run_re_establish_dr(ssl_context, token: str, dr_op_token: str):
    ...
    if is_re_establish_already_done(checks):
        return {
            "skipped": True,
            "message": "Re-establish DR already completed. No action required.",
            "checks": checks
        }

    a_stat = checks["primary_dr_status"]

    demote_skipped = True
    demote_reason = None

    if a_stat["http"] == 200 and a_stat["mode"] == "primary":
        print("Re-establish DR pre-step: demoting Cluster A because it is primary")
        ...
        require_ok("reestablish_step0_demote_cluster_a", st0, d0)
    ...
    print("Re-establish DR: Step 1 generate public key on Cluster A")
    st1, d1 = vault_post(
        ssl_context,
        PRIMARY_ADDR,
        token,
    ...
    require_ok("reestablish_step1_generate_public_key", st1, d1)

    pub = (d1.get("data") or {}).get("secondary_public_key")
    if not pub:
        raise Exception(json.dumps({
            "step": "reestablish_step1_generate_public_key",
    ...
    print("Re-establish DR: Step 2 generate activation token on Cluster B")
    st2, d2 = vault_post(
        ssl_context,
        SECONDARY_ADDR,
        token,
    ...
    require_ok("reestablish_step2_generate_activation_token", st2, d2)

    activation = (d2.get("data") or {}).get("token")
    if not activation:
        raise Exception(json.dumps({
            "step": "reestablish_step2_generate_activation_token",
            "error": "token missing",
            "response": d2
        }))
    ...
    print("Re-establish DR: Step 3 update primary on Cluster A")
    st3, d3 = vault_post(
        ssl_context,
        PRIMARY_ADDR,
        token,
    ...
    require_ok("reestablish_step3_update_primary", st3, d3)
...
def run_combined(validate_only: bool = False):
    ...
    controlled_result = run_controlled_failover(ssl_context, primary_token, failover_token)
    after_failover_checks = validate_current_state(ssl_context, primary_token)
    ...
    return {
        ...
        "mode": "execute",
        "initial_checks": initial_checks,
        "controlled_failover": controlled_result,
        "after_controlled_failover_checks": after_failover_checks,
