...
function configure_vault_server_logging {
  log "INFO" "Configuring Vault server logs from journald to /var/log/vault/vault-server.log"
...
function restart_zotelagent {
  ...
  # Directory: required for Vault audit log
  ...
  if [ ! -f /var/log/vault/vault-server.log ]; then
    sudo install -m 664 -o syslog -g "$VAULT_GROUP" /dev/null /var/log/vault/vault-server.log || true
  else
    sudo chown syslog:"$VAULT_GROUP" /var/log/vault/vault-server.log || true
    sudo chmod 664 /var/log/vault/vault-server.log || true
  fi

  sudo systemctl restart rsyslog || true
  sudo systemctl restart vault || true
...
main() {
  ...
  log "INFO" "Configuring vault server logs to kfuse"
  configure_vault_server_logging

  log "INFO" "patch role config with system and vault logs"
